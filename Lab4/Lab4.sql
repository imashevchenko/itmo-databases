...
(SELECT TOP 1 ProductID FROM Sales.SalesOrderDetail
 ...
 ORDER BY COUNT(*) DESC)
/*2.*/
SELECT TOP 1 CustomerID FROM Sales.SalesOrderHeader AS SOH
JOIN (SELECT SalesOrderID, SUM(OrderQty*UnitPrice) AS SUMMARY FROM Sales.SalesOrderDetail
...
SELECT Name FROM Production.Product AS P
WHERE (P.ListPrice> 
(SELECT AVG(ListPrice) FROM Production.Product AS P1
...
/*5.*/
SELECT OD.ProductID FROM Sales.SalesOrderDetail AS OD
JOIN Sales.SalesOrderHeader OH 
ON OD.SalesOrderID=OH.SalesOrderID 
WHERE OH.CustomerID IN 
(SELECT OH.CustomerID FROM Production.Product AS P 
...
/*6.*/
SELECT DISTINCT OD.ProductID FROM Sales.SalesOrderDetail AS OD
JOIN Sales.SalesOrderHeader OH 
ON OD.SalesOrderID=OH.SalesOrderID 
WHERE OH.CustomerID IN 
(SELECT OH.CustomerID FROM Production.Product AS P1 
JOIN Sales.SalesOrderDetail AS OD1
ON P1.ProductID=OD1.ProductID JOIN Sales.SalesOrderHeader OH1
...
SELECT COUNT(DISTINCT OD2.SalesOrderID) FROM Production.Product AS P2
JOIN Sales.SalesOrderDetail AS OD2 
ON P2.ProductID=OD2.ProductID JOIN Sales.SalesOrderHeader OH2
...
ON OD.SalesOrderID=OH.SalesOrderID
WHERE EXISTS(
SELECT OD1.ProductID FROM Sales.SalesOrderDetail AS OD1
...
WHERE OD1.ProductID=(
SELECT TOP 1 P2.ProductID FROM Production.ProductCategory AS PC
JOIN Production.ProductSubcategory AS PSC 
...
SELECT COUNT(OH3.SalesOrderId) FROM Sales.SalesOrderHeader AS OH3
...
/*10.*/
SELECT OH.CUSTOMERID FROM Sales.SalesOrderDetail AS OD
JOIN Sales.SalesOrderHeader AS OH
ON OH.SalesOrderID=OD.SalesOrderID
WHERE OD.SalesOrderID = ANY(
		SELECT OH.SalesOrderID FROM Sales.SalesOrderDetail AS OD
		JOIN Sales.SalesOrderHeader AS OH
		ON OH.SalesOrderID=OD.SalesOrderID
		WHERE OD.ProductID = ANY (
				SELECT ProductID FROM Sales.SalesOrderDetail AS OD2
...
/*13.*/
SELECT PSC.Name FROM Sales.SalesOrderDetail AS OD
JOIN Sales.SalesOrderHeader AS OH
ON OH.SalesOrderID=OD.SalesOrderID
...
(
SELECT ProductID FROM Sales.SalesOrderDetail AS OD
JOIN Sales.SalesOrderHeader AS OH
ON OH.SalesOrderID=OD.SalesOrderID
Group BY ProductID
