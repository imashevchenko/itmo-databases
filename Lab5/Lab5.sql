...
SELECT CustomerID, AVG(SalesByOrder)
FROM Sales.SalesOrderHeader AS OH
         JOIN SALES_NUMBER
              ON SALES_NUMBER.SalesOrderId = OH.SalesOrderID
GROUP BY CustomerID

SELECT CustomerID, AVG(OD.C)
FROM Sales.SalesOrderHeader AS OH
         JOIN (SELECT SalesOrderID, COUNT(*) as C
               FROM Sales.SalesOrderDetail
...
SELECT CustomerID,
       ProductID,
       CAST(CAST(COUNT(*) AS FLOAT) /
            CAST((SELECT AMOUNT FROM TMP WHERE TMP.CustomerID = OH.CustomerID) AS FLOAT) AS DECIMAL(6, 3)) AS ANSWER
...
SELECT TMP1.CustomerID, TMP1.MAXPRICE, TMP2.MINPRICE
FROM TMP1
         JOIN TMP2
...
HAVING COUNT(DISTINCT p.ProductId) = (SELECT COUNT(Distinct D.ProductId)
                                      FROM (SELECT ProductId
                                            FROM TMP1
                                            WHERE CustomerID = P.CustomerID
                                            GROUP BY ProductId
                                            HAVING COUNT(DISTINCT SalesOrderID) >= 2) AS D)
