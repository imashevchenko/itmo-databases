...
FROM (SELECT ProductID,
             ...
             AVG(StandardCost) OVER(PARTITION BY ProductSubcategoryID) AS Avg
      FROM Production.Product) AS P
WHERE StandardCost > P.Avg
  AND ProductSubcategoryID IS NOT NULL
/*5.*/
SELECT OD.ProductID,
       P.Name,
       AVG(OrderQty) OVER (PARTITION BY OD.SalesOrderID, OD.productID  ORDER BY OrderDate DESC
